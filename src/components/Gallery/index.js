...
class Gallery extends Component {
  ...
  onSelectThumbnail = id => {
    const selectedImage = imagesList.filter(eachImage => eachImage.id === id)
    this.setState({
      image: selectedImage[0].imageUrl,
      imageAltText: selectedImage[0].imageAltText,
    })
  }
          ...
          <ul className="thumbnails-container">
            {imagesList.map(eachImage => (
              <ThumbnailItem
                ...
                eachImage={eachImage}
                key={eachImage.thumbnailAltText}
                isActive={eachImage.imageUrl === image}
              />
            ))}
          </ul>
        </div>
